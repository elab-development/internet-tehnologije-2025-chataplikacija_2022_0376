...
export default function ChatWindow({
  ...
  const handleRemoveUser = async (userIdToRemove: string) => {
    ...
    try {
      await axios.post('/chats/group/remove-member', { chatId: conversationId, userId: userIdToRemove });
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      toast.error('Greška na serveru pri prijavi');
    }
  };

  const currentUserInChat = conversation?.participants?.find((p: any) => p.id === currentUser.id) as any;
  const amIAdmin = currentUserInChat?.chatRole?.toUpperCase() === 'ADMIN' || 
  ...
  if (loading) return <div className="h-full flex items-center justify-center bg-white"><Loader2 className="animate-spin text-blue-600" size={32} /></div>;
  if (!conversation) return <div className="h-full flex items-center justify-center">Čat nije pronađen.</div>;
    ...
    <div className="h-full flex flex-col bg-[#F0F2F5]">
            ...
            return (
              <MessageBubble 
                ...
                onEdit={(id) => {
                  ...
                  if (m?.type === 'text') setEditingMessage({ id: m.id, content: m.content });
                }} 
                onDelete={async (id) => {
                  if (confirm('Obrisati poruku?')) {
                    try { await axios.delete(`/messages/${id}`); toast.success('Poruka obrisana'); }
                    catch { toast.error('Greška pri brisanju'); }
                  }
                }} 
                onReport={(id) => { setReportingMessageId(id); setReportModalOpen(true); }}
              />
            );
          })
        )}
      ...
      <Modal isOpen={showInfoModal} onClose={() => setShowInfoModal(false)} title="Detalji grupe">
        <div className="p-2 space-y-6">
          <div className="text-center border-b pb-4"><h3 className="text-xl font-bold">{conversation?.name || "Grupa"}</h3></div>
          {amIAdmin && (
            <div className="flex gap-2 items-end bg-gray-50 p-3 rounded-lg border border-dashed border-gray-300">
              <div className="flex-1">
                <label className="text-xs text-gray-500 mb-1 block font-semibold">Dodaj člana (Email)</label>
                <input type="email" className="w-full border p-2 rounded text-sm focus:outline-none" placeholder="email@primer.com" value={newUserEmail} onChange={(e) => setNewUserEmail(e.target.value)} />
              </div>
              <Button onClick={handleAddUser} className="bg-blue-600 text-white h-[38px] px-4"><UserPlus size={18} /></Button>
            </div>
          )}
          <div>
            <h4 className="font-semibold mb-3 text-gray-700">Učesnici ({conversation?.participants?.length})</h4>
            <div className="space-y-2">
              {conversation?.participants?.map((participant: any) => {
                const isUserAdmin = participant.chatRole?.toUpperCase() === 'ADMIN' || participant.chatRole?.toUpperCase() === 'MODERATOR' || participant.role?.toUpperCase() === 'ADMIN' || participant.role?.toUpperCase() === 'MODERATOR';
                return (
                      ...
                      <div>
                        ...
                        {isUserAdmin && (
                          <span className="text-[10px] text-blue-600 font-bold flex items-center gap-1">
                            <Shield size={10} className="text-blue-600 fill-blue-50" /> MODERATOR
                          </span>
                        )}
                      </div>
                    </div>
                    {amIAdmin && participant.id !== currentUser.id && (
                      <button onClick={() => handleRemoveUser(participant.id)} className="text-red-400 hover:text-red-600 p-2"><Trash2 size={16} /></button>
                    )}
                  </div>
                );
              })}
      ...
      <Modal isOpen={reportModalOpen} onClose={() => setReportModalOpen(false)} title="Prijavi poruku">
        <div className="space-y-4 p-1">
          <Input label="Razlog" value={reportReason} onChange={(e) => setReportReason(e.target.value)} placeholder="Spam, Uvreda..." />
          <textarea className="w-full border p-2 rounded" rows={3} value={reportComment} onChange={(e) => setReportComment(e.target.value)} placeholder="Opis..." />
          <div className="flex gap-2 justify-end">
            <Button onClick={() => setReportModalOpen(false)}>Otkaži</Button>
            <Button className="bg-red-600 text-white" onClick={submitReport}>Prijavi</Button>
          </div>
        </div>
      </Modal>
    </div>
